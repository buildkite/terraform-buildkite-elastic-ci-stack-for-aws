...
| <a name="input_buildkite_agent_scaler_version"></a> [buildkite\_agent\_scaler\_version](#input\_buildkite\_agent\_scaler\_version) | Version of the buildkite-agent-scaler to use. | `string` | `"1.9.8"` | no |
...
| <a name="input_enable_pre_exit_disk_cleanup"></a> [enable\_pre\_exit\_disk\_cleanup](#input\_enable\_pre\_exit\_disk\_cleanup) | Controls whether disk cleanup also runs in the pre-exit hook after jobs complete. Disk cleanup always runs in the environment hook when disk space is low. When enabled, cleanup also runs in pre-exit to reclaim resources generated during job execution. | `bool` | `false` | no |
