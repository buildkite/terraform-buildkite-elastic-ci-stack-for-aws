...
| <a name="input_custom_user_data"></a> [custom\_user\_data](#input\_custom\_user\_data) | Optional - Unencoded custom user data for agent instances. When set, this replaces the module-managed Linux or Windows user data, so instance bootstrap inputs are not applied automatically. | `string` | `null` | no |
